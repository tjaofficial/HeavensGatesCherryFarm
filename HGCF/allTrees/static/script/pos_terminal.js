...
function openStripeManualCharge() {
    ...
    const totalText = els.totalAmount ? els.totalAmount.textContent : "";
    const saleNumber = state.currentSaleNumber || "Current POS Sale";
...
function updateButtons() {
    ...
    /*
        Hide real Stripe Terminal reader buttons for now.
        We are using manual Stripe Dashboard charging today.
    */
    setVisible(els.cardSaleBtn, false);
    setVisible(els.cancelCardPaymentBtn, false);
...
async function completeManualCardSale() {
    ...
    try {
        ...
        const response = await fetch(
            `${window.POS_CONFIG.manualCardCompleteUrlBase}${state.currentSaleId}/manual-card-complete/`,
            {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
        ...
        alert(`Sale ${data.sale_number} completed as manual card payment.`);
